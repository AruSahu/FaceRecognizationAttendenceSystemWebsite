...
def createFile():
    ...
    with open('innovators.csv', mode='w') as file:
                        ...
                        if best_class_probabilities > 0.87:
                            ...
                            for H_i in HumanNames:
                                if HumanNames[best_class_indices[0]] == H_i:
                                    ...
                                    print(result_names)
                                    if result_names not in temp:
                                        temp.append(result_names)
                                        numberValuei = numberValuei + 1
                                        # time2 = "9:30-10:30"
                                        # subject = "Python"
                                        # sem = 3
                                        # batch = 2019
                                        # year = 2022
                                        # date = "18-01-2022"  # to be autogenerated
                                        ...
                                        with open('innovators.csv', 'a', newline='') as file:
                                            ...
                                            # studentsPresent.append("{}".format(HumanNames[best_class_indices[0]]))
                                            # print(studentsPresent)
                                            # addStudentInTemp(studentsPresent[studentsPresent.count()-1])
                                            writer.writerow(["{}".format(HumanNames[best_class_indices[0]])])
                                            # writer.writerow([numberValuei, "{}".format(HumanNames[best_class_indices[0]]), datetime.datetime.now().strftime("%H:%M:%S")])

                                    # CSV WALA KAAM KARNA H
                                    # CSV WALA KAAM KARNA H
                                    # CSV WALA KAAM KARNA H
                                    # CSV WALA KAAM KARNA H
                                    # CSV WALA KAAM KARNA H
